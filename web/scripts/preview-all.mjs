...
function pad(c, p, W, H) {
	return { x: Math.max(0, c.x - p), y: Math.max(0, c.y - p), width: Math.min(W - Math.max(0, c.x - p), c.width + 2 * p), height: Math.min(H - Math.max(0, c.y - p), c.height + 2 * p) };
}
async function compose(page, name, title, beforeBuf, afterBuf, notes, w) {
	if (beforeBuf.length < 2500 || afterBuf.length < 2500) { console.error('SKIP ' + name + ': blank shot (b=' + beforeBuf.length + ' a=' + afterBuf.length + ')'); return false; }
	const b = beforeBuf.toString('base64'), a = afterBuf.toString('base64');
	await page.setViewportSize({ width: w * 2 + 60, height: 1400 });
	await page.evaluate(({ b, a, w, title, notes }) => {
...
// ---------- STAGE: default tab Screens -> Notes ----------
{
	...
	const stage = (await rectOf(page, '.main-stage, [class*="main-stage"], [class*="stage"]')) || { x: 410, y: 49, width: 1030, height: 760 };
	const clip = pad(stage, 0, W, H);
	const before = await page.screenshot({ clip });
	// proposed: Notes becomes the default-active tab → click Notes to show that state
	await page.evaluate(() => {
		const t = [...document.querySelectorAll('button[role="tab"], .tabbar button, [class*="tab"] button')].find((b) => /^\s*Notes/i.test(b.textContent));
		if (t) t.click();
	});
	await page.waitForTimeout(500);
	const after = await page.screenshot({ clip });
	await compose(page, 'preview-stage', 'Stage default tab — Screens → Notes (P0)', before, after,
		['Reference loads with the NOTES tab active: targeted[27] a#notes-tab.active, subtrees.presentation #notes.tab-pane.active.show',
		 'Fix: MainStage.svelte:63 $state(\'screens\') → \'notes\' (locked rooms still force Screens via the derived activeTab)'], clip.width);
	await browser.close();
}
...
// ---------- TOPNAV: user pill margin/height ----------
{
	...
	const after = await page.screenshot({ clip });
	await compose(page, 'preview-topnav', 'Topnav user pill — margin 5→4px, height 25→18px', before, after,
		['Reference: audit/topnav.json[48] margin-l/r = 4px (Bootstrap ml-1 mr-1), rect.h = 18px',
		 'Fix: RoomTopNav.svelte:275 margin 0 5px → 0 4px; :266 reduce gap so the pill resolves to 18px'], clip.width);
	await browser.close();
}
...
// ---------- CHAT: composer shape & metrics ----------
{
	...
	const pill = (await rectOf(page, '.chat-pane form .pill, .chat-pane .pill, [class*="chat"] form .pill')) || { x: 0, y: 840, width: 560, height: 56 };
	...
	await page.waitForTimeout(200);
	const after = await page.screenshot({ clip });
	await compose(page, 'preview-chat', 'Chat composer — pill→8px radius, textarea 30→35px / 300→400, placeholder #999', before, after,
		['Reference: #textAreaHolder border-radius 8px; .txt-area height/min-height 35px, weight 400; .form-control::placeholder #999999',
		 'Fix: ChatPanel.svelte:538 radius 999px→8px; :541-556 min-height 35px + weight 400; add ::placeholder #999999'], clip.width);
	await browser.close();
}
console.log('DONE');
