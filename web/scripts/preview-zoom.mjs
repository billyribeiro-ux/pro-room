...
async function composeZoom(page, name, title, clip, before, after, rows) {
	const b = before.toString('base64'),
		a = after.toString('base64');
	const dispW = clip.width * SCALE,
		dispH = clip.height * SCALE;
	await page.setViewportSize({ width: dispW * 2 + 60, height: dispH + 220 });
	await page.evaluate(
		({ b, a, dw, dh, title, rows }) => {
			document.documentElement.style.cssText = 'margin:0';
			document.body.style.cssText =
				"margin:0;background:#1b1b1b;color:#fff;font:14px/1.6 'Open Sans',sans-serif";
			const col = (t, c, img) =>
				`<div style="flex:0 0 ${dw}px"><div style="padding:6px 10px;background:${c};font-weight:700">${t}</div><img src="data:image/png;base64,${img}" style="display:block;width:${dw}px;height:${dh}px;image-rendering:-webkit-optimize-contrast;border:1px solid #333;background:#0c2434"></div>`;
			const table =
				'<table style="border-collapse:collapse;margin:10px 14px">' +
				...
				rows
					.map(
						(r) =>
							`<tr><td style="padding:2px 14px 2px 0">${r[0]}</td><td style="padding:2px 24px 2px 0;color:#e08a8a">${r[1]}</td><td style="color:#8ad28a">${r[2]}</td></tr>`
					)
					.join('') +
				'</table>';
			document.body.innerHTML =
				`<div style="padding:12px 14px 2px;font-weight:700;font-size:16px">${title}</div>` +
...
// ---------------- TOPNAV PILL ----------------
{
	...
	const cur = await measure(page, sel, [
		'height',
		'margin-left',
		'margin-right',
		'gap',
	...
	await page.addStyleTag({
		content: `.users, nav.topnav .users { margin-left:4px !important; margin-right:4px !important; gap:0 !important; height:18px !important; align-items:center !important; }`
	});
	await page.waitForTimeout(150);
	...
	await composeZoom(
		page,
		'preview-zoom-topnav',
		'Topnav user pill (zoom ×3) — ground truth: height 18px, margin 4px',
		clip,
		before,
		after,
		[
			[
				'height',
				(cur ? cur.height : '?') + '  (rect h=' + (cur && cur.__rect ? cur.__rect.h : '?') + ')',
				'18px'
			],
			[
				'margin-left / right',
				cur ? cur['margin-left'] + ' / ' + cur['margin-right'] : '?',
				'4px / 4px'
			],
...
// ---------------- CHAT COMPOSER ----------------
{
	...
	const curPill = await measure(page, pillSel, [
	...
	const clip = pr
		? {
				x: Math.max(0, pr.x - 6),
				y: Math.max(0, pr.y - 8),
				width: Math.min(W, pr.w + 12),
				height: pr.h + 16
			}
		: { x: 0, y: H - 70, width: 560, height: 64 };
	const before = await page.screenshot({ clip });
	await page.addStyleTag({
		content: `
	...
	await composeZoom(
		page,
		'preview-zoom-chat',
		'Chat composer (zoom ×3) — ground truth: #textAreaHolder radius 8px / border 0, .txt-area 35px / weight 400, placeholder #999',
		clip,
		before,
		after,
		[
