<script lang="ts">
	...
	const logs: { id: string; date: string; by: string; channel: string }[] = [];
	function reload() {
...
<Modal {open} {onClose} title="Chat Logs" {footer}>
	...
	<!-- Reference: a Bootstrap list-group of clickable action items (date / By / Channel).
	...
	<div class="list-group">
		{#each logs as log (log.id)}
			<button type="button" class="list-group-item">
				...
				<div class="lg-channel">Channel:&nbsp;{log.channel}</div>
			</button>
		{/each}
	</div>
</Modal>
